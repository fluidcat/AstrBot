...
class MessageMixin(WechatAPIClientBase):
    ...
    async def revoke_message(self, wxid: str, client_msg_id: int, create_time: int, new_msg_id: int) -> bool:
        ...
        async with aiohttp.ClientSession() as session:
            json_param = {"Wxid": self.wxid, "ToUserName": wxid, "ClientMsgId": client_msg_id, "CreateTime": create_time,
                          "NewMsgId": new_msg_id}
            response = await session.post(Uri.Revoke, json=json_param)
    ...
    async def _send_text_message(self, wxid: str, content: str, at: list[str] = None) -> tuple[int, int, int]:
        ...
        async with aiohttp.ClientSession() as session:
            ...
            response = await session.post(Uri.SendTxt, json=json_param)
            json_resp = await response.json()
            if json_resp.get("Success"):
    ...
    async def _send_image_message(self, wxid: str, image: Union[str, bytes, os.PathLike]) -> tuple[
        ...
        async with aiohttp.ClientSession() as session:
            ...
            response = await session.post(Uri.UploadImg, json=json_param)
            json_resp = await response.json()
    ...
    async def send_video_message(self, wxid: str, video: Union[str, bytes, os.PathLike],
        ...
        async with aiohttp.ClientSession() as session:
            json_param = {"Wxid": self.wxid, "ToWxid": wxid, "Base64": vid_base64, "ImageBase64": image_base64,
                          "PlayLength": duration}
            async with session.post(Uri.SendVideo, json=json_param) as resp:
    ...
    async def _send_link_message(self, wxid: str, url: str, title: str = "", description: str = "",
        ...
        app_msg_type = 5
        xml = f"""<appmsg><title>{title}</title><des>{description}</des><type>{app_msg_type}</type><action>view</action>
        <showtype>0</showtype><content></content><url>{url}</url><thumburl>{thumb_url}</thumburl></appmsg>"""

        async with aiohttp.ClientSession() as session:
            json_param = {"Wxid": self.wxid, "ToWxid": wxid, "Type": app_msg_type, "Xml": xml}

            response = await session.post(Uri.ShareLink, json=json_param)
            json_resp = await response.json()
    ...
    async def _send_emoji_message(self, wxid: str, md5: str, total_length: int) -> tuple[int, int, int]:
        ...
        async with aiohttp.ClientSession() as session:
            ...
            response = await session.post(Uri.SendEmoji, json=json_param)
            json_resp = await response.json()
    ...
    async def _send_card_message(self, wxid: str, card_wxid: str, card_nickname: str, card_alias: str = "") -> tuple[
        ...
        async with aiohttp.ClientSession() as session:
            ...
            response = await session.post(Uri.ShareCard, json=json_param)
            json_resp = await response.json()
    ...
    async def _send_app_message(self, wxid: str, xml: str, type: int) -> tuple[int, int, int]:
        ...
        async with aiohttp.ClientSession() as session:
            ...
            response = await session.post(Uri.SendApp, json=json_param)
            json_resp = await response.json()
    ...
    async def _send_cdn_file_msg(self, wxid: str, xml: str) -> tuple[int, int, int]:
        ...
        async with aiohttp.ClientSession() as session:
            ...
            response = await session.post(Uri.SendCDNFile, json=json_param)
            json_resp = await response.json()
    ...
    async def _send_cdn_img_msg(self, wxid: str, xml: str) -> tuple[int, int, int]:
        ...
        async with aiohttp.ClientSession() as session:
            json_param = {"Wxid": self.wxid, "ToWxid": wxid, "Content": xml}
            response = await session.post(Uri.SendCDNImg, json=json_param)
            json_resp = await response.json()
    ...
    async def _send_cdn_video_msg(self, wxid: str, xml: str) -> tuple[int, int]:
        ...
        async with aiohttp.ClientSession() as session:
            json_param = {"Wxid": self.wxid, "ToWxid": wxid, "Content": xml}
            response = await session.post(Uri.SendCDNVideo, json=json_param)
            json_resp = await response.json()
    ...
    async def sync_messag1e(self) -> dict:
        ...
        async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=10)) as session:
            ...
            response = await session.post(Uri.Sync, json=json_param)
            json_resp = await response.json()
